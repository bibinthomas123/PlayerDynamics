...
from explainability.xai_layer import XAILayer, format_episodic_context

def _make_ctx():
    ep = PlayerEpisode(
        episode_index=1, start_minute=22, end_minute=35,
        dominant_findings=["locomotor_overload"],
        trend_direction="worsening", severity="high",
        interventions=[], response="escalated",
        persistence_duration=4, peak_anomaly_score=0.8, peak_confidence=0.88,
    )
...
def test_episodic_block_in_prompt():
    ...
    with patch.object(xai._llm_nlg, '_available', True), \
         patch.object(xai._llm_nlg, '_circuit_open', False):
...
def test_format_episodic_context_empty_safe():
    ...
    empty = CompressedTemporalContext(
        trajectory_narrative="", trend_summaries={}, recent_context="",
        intervention_history=[], recurring_patterns=[], state_transitions=[],
        relevant_prior_episodes=[], current_escalation="normal", episode_count=0,
    )
